	...
	public void pesquisarPessoa(){
		try{
			...
			if(clienteCadastrado == null){
				FacesUtil.addMsgErro("Não há cliente cadastrado com o nome informado");
			}else{
	...
	public void devolver(){
		try{
			if(clienteCadastrado == null || listaLocacao.size() == 0){
				FacesUtil.addMsgErro("Impossível efetuar devolução! Pessoa não cadastrada na loja ou número há devoluções pendentes!");
			}else{
				LocacaoDAO dao2 = new LocacaoDAO();
				ServicoAcervoDAO dao3 = new ServicoAcervoDAO();
				...
				listaLocacao.clear();
				listaLocados.clear();
				FacesUtil.addMsgInfo("Devolução efetuada com sucesso!");
			}
		}catch(RuntimeException ex){
